...
def list_past_inferences(tool_context: ToolContext) -> Dict[str, Any]:
    """
    Scans the outputs directory for past inference reports and returns a numbered list.
    This list is stored in the agent's context for use with the
    'get_past_inference_recommendations' tool.

    Args:
        tool_context: The context object provided by the ADK agent.
    """
    if not os.path.exists(OUTPUTS_DIR):
        return {"status": "success", "message": "No past inference reports found."}

    files = [f for f in os.listdir(OUTPUTS_DIR) if f.startswith("inference_") and f.endswith(".json")]
    ...
    # Store the full file names in context for the next tool call
    tool_context.state["past_inferences_files"] = files

    # Create a user-friendly numbered list
    display_list = []
    for i, filename in enumerate(files):
        # Extract timestamp from filename: inference_YYYY-MM-DD_HH-MM-SS.json
        try:
            ts_str = filename.replace("inference_", "").replace(".json", "")
            dt_obj = datetime.datetime.strptime(ts_str, "%Y-%m-%d_%H-%M-%S")
            display_list.append(f"{i + 1}: Report from {dt_obj.strftime('%Y-%m-%d %H:%M:%S')}")
        except ValueError:
            display_list.append(f"{i + 1}: {filename}")

    return {
        "status": "success",
        "inferences_list": display_list
    }

def get_past_inference_recommendations(
    tool_context: ToolContext,
    inference_index: int
) -> Dict[str, Any]:
    """
    Retrieves the top recommendations from a specific past inference report,
    selected by its number from the list generated by 'list_past_inferences'.

    Args:
        tool_context: The context object provided by the ADK agent.
        inference_index: The 1-based number of the inference report to retrieve.
    """
    past_files = tool_context.state.get("past_inferences_files")
    if not past_files:
        return {"status": "error", "message": "You must run 'list_past_inferences' first to see the available reports."}

    # Convert 1-based index to 0-based
    index = inference_index - 1

    if not (0 <= index < len(past_files)):
        return {"status": "error", "message": f"Invalid index. Please choose a number between 1 and {len(past_files)}."}

    filename = past_files[index]
    ...
    try:
        ...
        recommendations = data.get("ranking_up")
        if recommendations is None:
            return {"status": "error", "message": f"Could not find 'ranking_up' key in report {filename}."}
            
        return {
            "status": "success",
            "message": f"Retrieved top recommendations from {filename}.",
            "recommendations": recommendations
        }
    except Exception as e:
        return {"status": "error", "message": f"Failed to read or parse file {filename}: {e}"}
